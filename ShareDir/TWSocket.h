...
typedef struct addritem_tag
{
	char szIpAddr[16];
    unsigned long nIpHost;
	unsigned long nIpNet;
} ADDRITEM, *LPADDRITEM;

class TWSocket  
{
protected:
	static    long      m_ObjectNums;         //对象的实例数,当ObjectNums==0时自动调用WSACleanup()
	                            //对象的实例数,当ObjectNums==1时自动调用WSAStartup()
	static    BOOL      m_InitSocketSuccess;  //是否已成功初始化WinSock.dll
	static    WSADATA   m_WSAData;            //成功初始化WinSock.dll时填充的WSADATA结构
public:
	SOCKET              m_Socket;
    int					FLastError;
public:
	...
	int              Detach();  
	                    //成功返回0 失败返回<0  错误号由GetLastError()返回
	static TWSocket*  FromHandle(SOCKET s);
	                    //成功返回类指针,否则返回NULL
	int              Accept(TWSocket* NewClass);
						...
						//成功返回0 失败返回<0  错误号由GetLastError()返回
    SOCKET           Accept();
	int              Bind(char *IpAddr=NULL,int Port=0);
		                //将一本地地址与套接字连接
	...
	int              WaitSend(const char* Buffer,int len,long DelayTime=0);
	                    //在已连接的套接字上发送数据
    ...
    int              Recv_Stream(char* Buffer,int len);
	                    ...
	                    //对方关闭时,返回0
	                    //出错返回<0  错误号由GetLastError()返回 
	int              WaitRecv(char* Buffer,int len,long DelayTime=0);
	                    //在已连接的套接字上读取输入数据
    ...
    int              GetSockName(char* IpAddrBuffer,int Bufferlen,int* Port);
	                    ...
	                    //Bufferlen为IpAddrBuffer缓冲区长度
	                    //Port中返回套接字本地IP端口
	int              SetSocketBlock(BOOL Blocking=true);
	                    //设置套接字为阻塞调用,缺省创建时为阻塞调用
	                    //Blocking=true  设置套接字为阻塞调用
	                    //Blocking=false 设置套接字为非阻塞调用
	                    //成功返回0 失败返回<0  错误号由GetLastError()返回

	int              SetRecvTimeOut(int ms);

	int              GetReadableChars(unsigned long *Len);
	                    //检测可从套接字中立即读出的数据量
	                    //成功返回0 失败返回<0  错误号由GetLastError()返回
	...
	static   void     IpHostToStr(unsigned long nIpHost, char *Address, int nSize);

	static   int      GetHostName(char *Name,int Len);  
	                    //得到本地主机名称
	                    //成功返回0 失败返回<0  错误号由GetLastError()返回
	static   BOOL     StrIsIP(const char *Address, unsigned long *lpIpHost=NULL); 
	                    //给定的字符串是否是数字IP
	...
	static   int       GetIPAddr(char* HostName,char *IPAddr,int IPAddrLen);
	                    ...
	                    //成功返回IP地址拷贝的字节数,
	                    //失败返回0
    static   int       GetLocalAllIPAddrs(LPADDRITEM lpAddrItems, int nLen);
	                    //成功返回地址项个数,可能为0, 一定不大于nLen
	                    //失败返回-1
	BOOL               ValidateSocket();
