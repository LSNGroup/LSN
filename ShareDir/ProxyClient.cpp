...
//////////////////////////////////
	...
	*pbSingleQuit = FALSE;
	...
	//Waiting...蚂蚁互动软件，必须在VncViewer退出时，立即退出这个Proxy函数！！！
#if 0
#ifdef WIN32
...
void ProxyClientStartSlave(SOCKET_TYPE ftype, SOCKET fhandle, SOCKET fudpsock, WORD wLocalTcpPort)
{
	int ret;
	sockaddr_in their_addr;
	int namelen = sizeof(their_addr);

	if (ftype != SOCKET_TYPE_UDT) {
		return;
	}
	...
	if (UDT::ERROR == UDT::bind2(serv_slave, fudpsock))
	{
