...
rfbBool rfbFilenameTranslate2UNIX(rfbClientPtr cl, char *path, char *unixPath)
{
    ...
    else if (path[0]=='S' && path[1]==':')
    {
    	strcpy(unixPath, "/sdcard");
    	strcat(unixPath, &path[2]);
    }
    else
    {
      home = getenv("HOME");
...
rfbBool rfbSendDirContent(rfbClientPtr cl, int length, char *buffer)
{
    ...
    g2u(buffer, strlen(buffer), path_tmp, sizeof(path_tmp));
    rfbFilenameTranslate2UNIX(cl, path_tmp, path);
    ...
    for (direntp=readdir(dirp); direntp!=NULL; direntp=readdir(dirp))
    {
        ...
        if (retval==0)
        {
            ...
            /* u2g(direntp->d_name, strlen(direntp->d_name), path_tmp, sizeof(path_tmp)); */
            strcpy((char *)win32filename.cFileName, "<!UTF-8!>");
            strcat((char *)win32filename.cFileName, direntp->d_name);
            
            /* Do not show hidden files (but show how to move up the tree) */
            if ((strcmp(direntp->d_name, "..")==0) || (strcmp(direntp->d_name, ".")!=0))  //Changed by Gaohua
            {
    ...
    */
    ...
    case rfbDirContentRequest:
        ...
        case rfbRDrivesList: /* Client requests the List of Local Drives */
            ...
            */
            ...
            filename2[7]=0;
            filename2[8]=0;
            retval = rfbSendFileTransferMessage(cl, rfbDirPacket, rfbADrivesList, 0, 9, filename2);
            if (buffer!=NULL) free(buffer);
            return retval;
            break;
    ...
    case rfbFileTransferRequest:
        ...
        */
        ...
        g2u(buffer, strlen(buffer), filename_tmp, sizeof(filename_tmp));
        rfbFilenameTranslate2UNIX(cl, filename_tmp, filename1);
        ...
        */
        ...
        if (cl->fileTransfer.fd!=-1) {
            if (fstat(cl->fileTransfer.fd, &statbuf)!=0) {
            	...
            	rfbLog("rfbProcessFileTransfer() rfbFileTransferRequest: Is a directory!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
                close(cl->fileTransfer.fd);
                cl->fileTransfer.fd=-1;
            }
            else
        ...
        */
        ...
        retval = rfbSendFileTransferMessage(cl, rfbFileHeader, 0, (cl->fileTransfer.fd==-1 ? 0xffffffffu : statbuf.st_size), length, buffer);

        /* TODO: finish 64-bit file size support */
        sizeHtmp = (cl->fileTransfer.fd==-1 ? 0xffffffffu : 0);
        LOCK(cl->sendMutex);
        ...
        case rfbCFileRename: /* Client requests the Renaming of a file/directory */
            ...
            if (p != NULL)
            {
                ...
                *p = '\0';
                g2u(buffer, strlen(buffer), filename_tmp, sizeof(filename_tmp));
                rfbFilenameTranslate2UNIX(cl, filename_tmp, filename1);
                g2u(p+1, strlen(p+1), filename_tmp, sizeof(filename_tmp));
                rfbFilenameTranslate2UNIX(cl, filename_tmp, filename2);
...
rfbProcessClientNormalMessage(rfbClientPtr cl)
{
	...
	cl->readyForSetColourMapEntries = TRUE;
	  ...
	  if (cl->screen->width * cl->screen->height >= 720*1280 && cl->scaledScreen == cl->screen) {
	  	  int init_scale = 2;
	  	  if (cl->screen->width * cl->screen->height >= 1080*1920) {
	  	  	  init_scale = 3;
	  	  }
	  	  if (cl->screen->width * cl->screen->height >= 2560*1440) {
	  	  	  init_scale = 4;
	  	  }
	      L("Init cl->screen->width=%d  cl->screen->height=%d  rfbSetScale(%d)\n", cl->screen->width, cl->screen->height, init_scale);
	      rfbScalingSetup(cl, cl->screen->width/init_scale, cl->screen->height/init_scale);
    ...
    case rfbSetEncodings:
    {
		...
		if (!cl->useNewFBSize) {
		    L("Can not enabling NewFBSize protocol extension for client "
			   "%s\n", cl->host);
	...
	if(!cl->viewOnly) {
		char tmp[1024*2];
		g2u(str, strlen(str), tmp, sizeof(tmp));
	    cl->screen->setXCutText(tmp, strlen(tmp), cl);
	}
	free(str);
