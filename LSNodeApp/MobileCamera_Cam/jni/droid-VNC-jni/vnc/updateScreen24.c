...
static signed char is_a_equal_b(uint8_t* pa, int offset_a, uint8_t* pb, int offset_b)
{
	if (pa[offset_a + 0] == pb[offset_b + 0] && pa[offset_a + 1] == pb[offset_b + 1] && pa[offset_a + 2] == pb[offset_b + 2]) {
		return 1;
	}
	else {
		return 0;
...
static void set_a_with_b(uint8_t* pa, int offset_a, uint8_t* pb, int offset_b)
{
	pa[offset_a + 0] = pb[offset_b + 0];
	pa[offset_a + 1] = pb[offset_b + 1];
	pa[offset_a + 2] = pb[offset_b + 2];
}


void update_screen_24(void)
{  
  int i,j,r;
  ...
  if (method==FRAMEBUFFER) {
    ...
    b = (uint8_t*) readBufferFB();
  }
  else if (method==ADB)
    b = (uint8_t*) readBufferADB();
  else if (method==FLINGER)
    b = (uint8_t*) readBufferFlinger();
  ...
  if (rotation==0) {
    for (j = 0; j < vncscr->height; j++) {
      for (i = 0; i < vncscr->width; i++) {
        ...
        if (!is_a_equal_b(a,i + offset,b,pixelToVirtual)) {
          set_a_with_b(a,i + offset,b,pixelToVirtual);
          if (i>max_x)
          max_x=i;
          if (i<min_x)
          min_x=i;
    ...
    for (j = 0; j < vncscr->width; j++) {
      for (i = 0; i < vncscr->height; i++) {
        ...
        if (!is_a_equal_b(a,(vncscr->width - 1 - j + offset),b,pixelToVirtual))
        {
          set_a_with_b(a,(vncscr->width - 1 - j + offset),b,pixelToVirtual);
    ...
    for (j = 0; j < vncscr->height; j++) {
      for (i = 0; i < vncscr->width; i++) {
        ...
        if (!is_a_equal_b(a,((vncscr->width - 1 - i) + offset ),b,pixelToVirtual)) {
          set_a_with_b(a,((vncscr->width - 1 - i) + offset ),b,pixelToVirtual);
    ...
    for (j = 0; j < vncscr->width; j++) {
      for (i = 0; i < vncscr->height; i++) {
        ...
        if(!is_a_equal_b(a,j + offset,b,pixelToVirtual)) {
          set_a_with_b(a,j + offset,b,pixelToVirtual);
