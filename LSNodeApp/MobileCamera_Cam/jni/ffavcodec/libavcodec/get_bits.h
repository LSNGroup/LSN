...
#   define UPDATE_CACHE(name, gb)\
    if(name##_bit_count >= 0){\
		if (name##_buffer_ptr + 2 <= (gb)->buffer_end) {\
			name##_cache+= AV_RB16(name##_buffer_ptr) << name##_bit_count; \
		}\
		else if (name##_buffer_ptr + 1 <= (gb)->buffer_end) {\
			uint16_t name##_temp_word = AV_RB8(name##_buffer_ptr);\
			name##_cache+= AV_RB16(&(name##_temp_word)) << name##_bit_count; \
		}\
        name##_buffer_ptr+=2;\
        name##_bit_count-= 16;\
    }\
