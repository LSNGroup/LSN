...
{
	return (ipc_client_sock != INVALID_SOCKET);
}
	...
	while (1)
	{
		...
		if (timeoutMedia > 0 && currentLastMediaTime > 0 && currentSourceIndex != -1)
		{
			...
			if (nowTime - currentLastMediaTime > timeoutMedia) {
				__android_log_print(ANDROID_LOG_INFO, "viewer_jni", "CheckMediaThreadFn: timeout!!! %lu > %lu(timeoutMedia)\n", nowTime - currentLastMediaTime, timeoutMedia);
...
void ReturnViewerNode(VIEWER_NODE *pViewerNode)
{
	...
	if (nConnections <= 1 && is_app_connected())
	{
		__android_log_print(ANDROID_LOG_INFO, "viewer_jni", "ReturnViewerNode: Need new connection... nConnections=%d", nConnections);
...
void *IpcServerThreadFn(void *pvThreadParam)
{
	...
	while (1)
	{
		...
		__android_log_print(ANDROID_LOG_INFO, "viewer_jni", "IpcServerThreadFn: TCP accept() OK...\n");
