...
public:
	void*				get_user_ptr(int32_t rudp_id);
	void				set_user_ptr(int32_t rudp_id, void* ptr);
...
#define rudp_get_user_ptr(rudp_id) \
	RUDP()->get_user_ptr(rudp_id)

#define rudp_set_user_ptr(rudp_id, ptr) \
	RUDP()->set_user_ptr(rudp_id, ptr)

#define rudp_get_peer_addr(rudp_id, addr) \
	RUDP()->get_peer_addr(rudp_id, addr)
