	...
	if (g_bDoConnection1)
	{
		...
		if (ret == 3) {
			...
			if (strcmp(myHttpOperate.m1_event_type, "Accept") == 0)  /* Accept */
			{
				...
				g_hWorkingThread1 = 0;
				__android_log_print(ANDROID_LOG_INFO, "NativeMainFunc", "WorkingThreadFn1 ends...\n");
			...
			else if (strcmp(myHttpOperate.m1_event_type, "AcceptRev") == 0)  /* AcceptRev */
			{
				...
				__android_log_print(ANDROID_LOG_INFO, "NativeMainFunc", "WorkingThreadFnRev ends...\n");
...
void StartDoConnection()
{
	...
	//DoPush
	pthread_t hThread;
	pthread_create(&hThread, NULL, NativeMainFunc, NULL);
}
...
void StopDoConnection()
{
	...
	{
		int ret = myHttpOperate.DoPushEnd(g_client_charset, g_client_lang, push_channel_id);
		__android_log_print(ANDROID_LOG_INFO, "StopDoConnection", "DoPushEnd(%ld) = %d\n", push_channel_id, ret);
		if (ret != -1) push_channel_id = 0;
	}
