...
import org.droidplanner.mission.MAVLinkClient.OnMavlinkClientListener;
...
import com.wangling.remotephone.MobileCameraActivity;
 ...
 * 
 */
public class WaypointMananger implements OnMavlinkClientListener {
	
	MobileCameraActivity myDrone;
	...
	public void getWaypoints() {
		...
		MavLinkWaypoint.requestWaypointsList(myDrone.m_MAVLinkConnection);
	}
	...
	public void writeWaypoints(List<msg_mission_item> data) {
		...
		if ((mission != null)) {
			...
			myDrone.MavClient.setTimeOut();
			MavLinkWaypoint.sendWaypointCount(myDrone.m_MAVLinkConnection, mission.size());
		}
	}
	...
	public void setCurrentWaypoint(int i) {
		if ((mission != null)) {
			MavLinkWaypoint.sendSetCurrentWaypoint(myDrone.m_MAVLinkConnection, (short) i);
		}
	}
	...
	public WaypointMananger(MobileCameraActivity drone) {
		myDrone = drone;
		state = waypointStates.IDLE;
		wpEventListener = null;
	}
	...
	public boolean processMessage(MAVLinkMessage msg) {
		...
		case READ_REQUEST:
			if (msg.msgid == msg_mission_count.MAVLINK_MSG_ID_MISSION_COUNT) {
				...
				mission.clear();
				myDrone.MavClient.setTimeOut();
				MavLinkWaypoint.requestWayPoint(myDrone.m_MAVLinkConnection, mission.size());
		...
		case READING_WP:
			if (msg.msgid == msg_mission_item.MAVLINK_MSG_ID_MISSION_ITEM) {
				...
				if (mission.size() < waypointCount) {
					...
					state = waypointStates.IDLE;
					MavLinkWaypoint.sendAck(myDrone.m_MAVLinkConnection);
					myDrone.onMissionReceived(mission);
		...
		case WAITING_WRITE_ACK:
			if (msg.msgid == msg_mission_ack.MAVLINK_MSG_ID_MISSION_ACK) {
				myDrone.MavClient.resetTimeOut();
				myDrone.onWriteWaypoints((msg_mission_ack) msg);
				state = waypointStates.IDLE;
	...
	public boolean notifyTimeOut(int mTimeOutCount) {
		...
		case WRITTING_WP:
			...
			if (writeIndex < mission.size()) {
				myDrone.m_MAVLinkConnection.sendMavPacket(mission.get(writeIndex).pack());
			}
			break;
		case WAITING_WRITE_ACK:
			myDrone.m_MAVLinkConnection.sendMavPacket(mission.get(mission.size() - 1).pack());
			break;
		}
