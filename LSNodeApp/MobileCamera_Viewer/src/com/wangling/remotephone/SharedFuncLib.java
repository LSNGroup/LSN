...
public class SharedFuncLib {
    ...
    public static short getUint16Val(byte[] buff, int offset)
    {
    	short ret = (short)(((buff[offset] << 8 ) & 0xff00) | ((buff[offset + 1]) & 0x00ff));
    	return ret;
    }
	
    public static int getUint32Val(byte[] buff, int offset)
    {
    	int ret = ((buff[offset] << 24 ) & 0xff000000) 
			    	| ((buff[offset + 1] << 16 ) & 0x00ff0000) 
			    	| ((buff[offset + 2] << 8 ) & 0x0000ff00) 
			    	| ((buff[offset + 3]) & 0x000000ff);
    	return ret;
    }
	...
	public static native double TLVRecvGetData(int tlv_type);
